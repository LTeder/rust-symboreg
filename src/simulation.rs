...
impl Simulation {
    ...
    pub fn run(&mut self, debug_level: usize, skip: usize) {
        ...
        for i in (0..self.iterations).progress() {
            ...
            if (i + 1) % skip == 0 {
                debug_print(debug_level, i + 1, &population, &champion, &challenger);
            }
...
fn debug_print(debug_level: usize, epoch: usize, population: &[Individual],
               champion: &Individual, challenger: &Individual) {
        if debug_level == 1 {
            println!("{}, {}, {}", epoch, champion.fitness, challenger.fitness);
        } else if debug_level == 2 {
            println!("\nepoch {}\nchampion fitness: {}\nchallenger fitness: {}",
                     epoch, champion.fitness, challenger.fitness);
